...
BACKTEST_RESULTS_FILE = Path(__file__).parent / "data" / \
                             "backtest_results" / "backtest_full_1971_2025.csv"
BACKTEST_VALIDATION_FILE = Path(__file__).parent / "data" / \
...
def verify_uploads(db):
    ...
    if fred_count > 0:
        ...
        for series in series_list[:5]:
            print(
                f"  - {series['series_id']}:"
                f" {series['total_points']} pts"
                f" ({series['start_date']}"
                f" to {series['end_date']})"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Seed Azure Tables with FRED data and backtest results")
    ...
    parser.add_argument("--verify", action="store_true",
                        help="Only verify existing data (no upload)")
