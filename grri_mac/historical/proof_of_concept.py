...
from grri_mac.historical.mac_historical import run_historical_backtest
...
def analyze_crisis_periods(indicators: Dict, results: List):
    ...
    for crisis in crises:
        ...
        if nearest_result and min_diff <= 90:  # Within 90 days
            ...
            print(
                f"  {crisis['date'].strftime('%Y-%m')}"
                f" | {crisis['name']:30s}"
                f" | MAC: {mac:.2f} ({status:11s})"
                f" | {warning}"
            )
            print(
                f"           | Severity: {crisis['severity']:8s} | Regime: {nearest_result.regime}")
    ...
    if total_with_data > 0:
        ...
        print(
            f"\n  Summary: {warnings_detected}/{total_with_data}"
            f" crises had prior warning"
            f" ({warning_rate:.0f}%)"
        )
