...
def calculate_momentum(
    ...
    if is_deteriorating and momentum_4w is not None:
        warning_message = (
...
def analyze_momentum_around_crises(
    ...
    for crisis_date_str, crisis_name in crisis_dates:
        ...
        mac_4w = None
        for delta in range(14):
            check = (datetime.strptime(date_4w, "%Y-%m-%d") +
...
if __name__ == "__main__":
    ...
    print("\n  Detection Rates:")
    level_w = results['level_warnings']
    trend_w = results['trend_warnings']
    enh_w = results['enhanced_warnings']
    total = results['crises_analyzed']
    lvl_r = results.get('level_rate', 0) * 100
    trd_r = results.get('trend_rate', 0) * 100
    enh_r = results.get('enhanced_rate', 0) * 100
    print(
        f"    Level-based (MAC < 0.50):     "
        f"{level_w}/{total} ({lvl_r:.0f}%)"
    )
    print(
        f"    Trend-based (declining >0.05): "
        f"{trend_w}/{total} ({trd_r:.0f}%)"
    )
    print(
        f"    Enhanced (either):             "
        f"{enh_w}/{total} ({enh_r:.0f}%)"
    )
